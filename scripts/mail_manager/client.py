...
class MailClient:
    ...
    def __init__(self, account_config: dict[str, Any]) -> None:
        ...
        self.email: Optional[str] = self.config.get('EMAIL')
    ...
    def _get_mailbox(self) -> MailBox:
        ...
        try:
            print(f"Connecting to {imap_server}:{imap_port} (is_netease={any(domain in str(imap_server).lower() for domain in ['163.com', '126.com', 'yeah.net'])}, use_ssl={use_ssl})")
            ...
            is_netease = any(domain in str(imap_server).lower() for domain in ['163.com', '126.com', 'yeah.net'])
            if is_netease:
                try:
                    mailbox.client._simple_command('ID', '("name" "PythonMailClient" "version" "1.0")')  # type: ignore[attr-defined]
                except Exception:
    ...
    def fetch_emails(
        ...
        db_check_func: Optional[Callable[[str], bool]] = None
    ) -> list[dict[str, Any]]:
        """
                ...
                for current_folder in folders_to_fetch:
                    ...
                    for msg in mailbox.fetch(search_criteria, limit=limit, reverse=True, mark_seen=False):
                        ...
                        if references_header:
                            ...
                            if not refs:
                                refs = [x.strip() for x in str(references_header).split() if x.strip()]
                        in_reply_to_norm = ''
                        if in_reply_to_header:
                            ...
                            if m.startswith('<') and m.endswith('>'):
                                m = m[1:-1]
                            in_reply_to_norm = m
                        global_msg_id = str(msg_id_header) if msg_id_header else f"{self.email}-{current_folder}-{msg.uid}"
    ...
    def mark_as_starred(self, uids: list[str], folder: str = 'INBOX', is_starred: bool = True) -> None:
        """
    ...
    def move_emails(self, uids: list[str], destination_folder: str, source_folder: str = 'INBOX') -> None:
        """
